...
class StyledLexerBatch(QsciLexerBatch):
    def defaultFont(self, p_int):
        font: QFont = super().defaultFont(p_int)
        font_family = current_styles.editor_batch.get('font', {}).get('default', None)
        if font_family is not None:
            font.setFamily(font_family)
        return font

    def defaultPaper(self, style: int) -> QColor:
        return QColor(current_styles.editor_batch['paper']['background'])

    def defaultColor(self, style: int) -> QColor:
        color = current_styles.get_editor_color(current_styles.editor_batch.get('color'), style)
        if color:
            return QColor(color)
        return QColor('')


@register(file_types=['bat'])
class SQlCodeWidget(TabCodeWidget):
    file_type = 'bat'

    def set_lexer(self) -> Any:
        return StyledLexerBatch(self)
